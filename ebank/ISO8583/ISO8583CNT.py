...
(C) Copyright 2016 Manuel Vega Ulloa
...
__author__ = 'Manuel Vega Ulloa <manuel.vega@manexware.com>'
__version__ = '1.0.0'
__licence__ = 'GPL V3'
...
"""
    ...
    _BITS_VALUE_TYPE = {1: ['BME', 'Bit Map Extended', 'B', 16, 'b'],
                        ...
                        3: ['3', 'Precessing code', 'N', 6, 'n'], 4: ['4', 'Amount transaction', 'N', 20, 'n'],
                        5: ['5', 'Amount reconciliation', 'N', 12, 'n'],
                        6: ['6', 'Amount cardholder billing', 'N', 12, 'n'],
                        7: ['7', 'Date and time transmission', 'N', 14, 'n'],
                        ...
                        28: ['28', 'Date reconciliation', 'LL', 8, 'an'],
                        ...
                        37: ['37', 'Retrieval reference number', 'N', 12, 'n'],
                        38: ['38', 'Approval code', 'N', 6, 'an'], 39: ['39', 'Action code', 'A', 2, 'an'],
                        40: ['40', 'Service code', 'N', 3, 'an'], 41: ['41', 'Terminal', 'LL', 16, 'an'],
                        ...
                        45: ['45', 'Track 1 data', 'LL', 35, 'n'], 46: ['46', 'Amounts fees', 'LLL', 999, 'an'],
                        47: ['47', 'Additional data national', 'LLL', 999, 'an'],
                        48: ['48', 'Additional data private', 'LLL', 999, 'an'],
                        49: ['49', 'Verification data', 'AN', 3, 'an'],
                        50: ['50', 'Currency code, settlement', 'AN', 3, 'an'],
                        51: ['51', 'Currency code, cardholder billing', 'LL', 20, 'an'],
                        52: ['52', 'Personal identification number (PIN) data', 'N', 8, 'n'],
                        53: ['53', 'Security related control information', 'N', 16, 'n'],
                        54: ['54', 'Amounts additional', 'N', 16, 'n'],
                        55: ['55', 'Integrated circuit card (ICC) system related data', 'N', 16, 'n'],
                        56: ['56', 'Original data elements', 'N', 16, 'n'],
                        57: ['57', 'Authorisation life cycle code', 'N', 4, 'n'],
                        58: ['58', 'Authorising agent institution identification code', 'N', 8, 'n'],
                        59: ['59', 'Transport data', 'N', 2, 'n'],
                        60: ['60', 'Reserved for national use', 'LLL', 999, 'ans'],
                        61: ['61', 'Reserved for national use', 'N', 20, 'n'],
                        62: ['62', 'Reserved for private use', 'LL', 99, 'ans'],
                        63: ['63', 'Reserved for private use', 'N', 27, 'n'],
                        ...
                        69: ['69', 'Settlement institution county code', 'LL', 60, 'an'],
    ...
    def __init__(self, iso="", debug=False):
        ...
        @param: iso a String that represents the ASCII of the package. The same that you need to pass to setIsoContent()
            method.
        @param: Debug (True or False) default False -> Used to print some debug info. Only use if want that messages!
        """
    ...
    def set_bit(self, bit, value):
        ...
        It's one of the most important methods to use when using this library
    ...
    def __initialize_bits_from_bitmap_str(self, bitmap=None):
    ...
    def __set_bit_type_ll(self, bit, value):
        ...
        Example: pack.setBit(99,'123') -> Bit 99 is a LL type, so this bit, in ASCII form need to be 03123.
    ...
    def __set_bit_type_lll(self, bit, value):
        ...
        It put the size in front of the value
        Example: pack.setBit(104,'12345ABCD67890') -> Bit 104 is a LLL type, so this bit, in ASCII form need
    ...
    def __set_bit_type_n(self, bit, value):
        ...
        Example: pack.setBit(3,'30000') -> Bit 3 is a N type, so this bit, in ASCII form need to have size = 6
    ...
    def __set_bit_type_a(self, bit, value):
        ...
        Example: pack.setBit(3,'30000') -> Bit 3 is A type, so this bit, in ASCII form need to have size = 6
            (ISO specification) so the value 30000 size = 5 need to receive more "1" number.
            In this case, will be "0" in the left. In the package, the bit will be sent like '030000'
        @param: bit -> bit to be setted
        @param: value -> value to be setted
    ...
    def __set_bit_type_b(self, bit, value):
        ...
        It complete the size of the bit with a default value
        Example: pack.setBit(3,'30000') -> Bit 3 is a B type, so this bit, in ASCII form need to have size = 6
            (ISO specification) so the value 30000 size = 5 need to receive more "1" number.
            In this case, will be "0" on the left. In the package, the bit will be sent like '030000'
        @param: bit -> bit to be setted
        @param: value -> value to be setted
        @raise: ValueToLarge Exception
        It's a internal method, so don't call!
    ...
    def __set_bit_type_ans(self, bit, value):
        ...
        It completes the size of the bit with a default value
        Example: pack.setBit(3,'30000') -> Bit 3 is a ANS type, so this bit, in ASCII form need to have size = 6
            (ISO specification) so the value 30000 size = 5 need to receive more "1" number.
            In this case, will be "0" in the left. In the package, the bit will be sent like '030000'
        @param: bit -> bit to be setted
        @param: value -> value to be setted
        @raise: ValueToLarge Exception
        It's an internal method, so don't call!
        """
    ...
    def redefine_bit(self, bit, small_str, large_str, bit_type, size, value_type):
        """Method that redefine structure a bit in global scope!
        Can be used to personalize ISO8583 structure to another specification (ISO8583 1987 for example!)
        Hint: If you have a lot of "ValueToLarge Exception" maybe the specification that you are using is different of
            mine. So you will need to use this method :)
        @param: bit -> bit to be redefined
        @param: smallStr -> a small String representation of the bit, used to build "user-friendly prints",
            example "2" for bit 2
        @param: largeStr -> a large String representantion of the bit, used to build "user-friendly prints" and to be
            used to inform the "main use of the bit",
            example "Primary account number (PAN)" for bit 2
        @param: bitType -> type the bit, used to build the values, example "LL" for bit 2. Need to be one of
            (B, N, AN, ANS, LL, LLL)
        @param: size -> limit size the bit, used to build/complete the values, example "19" for bit 2.
        @param: valueType -> value types the bit, used to "validate" the values, example "n" for bit 2.
            This means that in bit 2 we need to have only numeric values.
    ...
    def __set_mti_from_str(self, iso):
        """Method that get the first four characters to be the MTI.
        It's a internal method, so don't call!
        """
    ...
    def __get_bit_from_str(self, str_without_mti_bitmap):
        ...
        offset = 0
        # jump bit 1 because it was already defined in the "__initialize_bits_from_bitmap_str"
    ...
    def __cmp__(self, obj2):
        ...
        if (self.get_mti() == obj2.getMTI()) and (self.get_bitmap() == obj2.getBitmap()) and (
                self.get_values_array() == obj2.getValuesArray()):
            ret = 0
    ...
    def get_bit(self, bit):
        ...
        @param:  -> the number of the bit that you want the value
    ...
    def set_network_iso(self, iso, big_endian=True):
        """Method that receive sie + ASCII ISO8583 package and transform it in the ISO8583 object.
            By default, it receive the package with size represented with big-endian.
            Is the same that:
            import struct
            (...)
            iso = ISO8583()
